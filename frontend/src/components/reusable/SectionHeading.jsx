...
const SectionHeading = ({ backgroundText = '', heading, subtext = '', align = 'center', py='py-40'}) => {
  const isCentered = align === 'center';

  return (
    <section>
      <div className={`relative w-full px-4 py-10 sm:py-20 lg:py-${py} overflow-hidden sm:mt-0 mt-10 text-${align}`}>
        ...
        <div className={`${isCentered ? 'absolute top-1/2 left-1/2 -translate-x-1/2 translate-y-[15%] md:translate-y-[55%]' : ''} z-10`}>
          <h2 className={`inline text-base sm:text-2xl md:text-3xl lg:text-4xl font-semibold text-secondary-blue ${!isCentered ? 'text-left' : ''}`}>
            {heading}
          </h2>
        </div>
      </div>
      ...
      {/* Subtext */}
      {subtext && (
        <div className={`pb-10 px-4 ${isCentered ? 'text-center' : 'text-left'}`}>
